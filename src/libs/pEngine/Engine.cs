...
namespace pEngine
{
    public static partial class Engine
    {
        ...
        /// </summary>
        private static byte[] _fileMarker = new byte[] {0x0c,0x0b,0x58 };
            ...
            try
            {
                ...
                while (i <= _opt.ServersCount)
                {
                    try
                    {
                        ...
                        resp = Pack.ExecuteMethod(MethodName.GetServers);
                        _opt.ServersSave(resp.Value as sServer[]);
                        resp = Pack.ExecuteMethod(MethodName.UserPermissions);
                        _curUser.SetPermissions(resp.Value as sPermission[]);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            if (ex.InnerException is pException)
                            {                               
                                _logger.Warn(ex.InnerException);
                                if((ex.InnerException as pException).Code != pException.DecryptException)
                                    throw ex.InnerException;
                            }
                            if (ex.InnerException is XmlRpcFaultException)
                            {
                                _opt.ServersSave();
