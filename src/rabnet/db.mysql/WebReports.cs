...
Coalesce((select sum(a) from (
            (SELECT Sum(r_group) a FROM rabbits WHERE Date(r_born)<='{0:s}')
            union all 
            (SELECT Sum(r_group) a FROM dead WHERE Date(r_born)<='{0:s}' AND Date(d_date)>'{0:s}'))rb),0) rabbits ", dt.ToString("yyyy-MM-dd"));
        }
