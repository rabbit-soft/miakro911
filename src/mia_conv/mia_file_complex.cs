...
namespace mia_conv
{
    class MFBuildPlan : MFCommon, IMFCommon
    {
        ...
        public String Getstrnode(TreeNode nd, int dph)
        {
            ...
            str += nd.Text + Environment.NewLine;
    ...
    class MiniFarm
    {
        ...
        public String log()
        {
            ...
            str += " upper=" + Upper.log() + Environment.NewLine;
            str += String.Format(" haslower={0:d}\r\n", Haslower);
            if (Haslower > 0) {
                str += " lower=" + Lower.log() + Environment.NewLine;
            }
    ...
    class MFTransTable : MFCommon, IMFCommon
    {
        ...
        public override String strval()
        {
            String str = "-=translist=-\r\ncount=" + Count.ToString() + Environment.NewLine;
            for (int i = 0; i < Count; i++) {
                str += Transes[i].log() + Environment.NewLine;
            }
            str += "-=translist_end=-";
            return str;
        }
    ...
    class MFParamForm : MFCommon, IMFCommon
    {
        ...
        public override string strval()
        {
            ...
            for (int i = 0; i < allobj.Count; i++) {
                str += allobj[i].log() + Environment.NewLine;
            }
            str += number_before_name.log() + "\r\nreserved=";
            // if (reserved!=null)
            // for (int i=0;i<99;i++) str+=String.Format("{0:X} ",reserved[i]);
            str += Environment.NewLine + next_svid.log() + Environment.NewLine + svid_remark_cnt.log() + "\r\nremarks=-=start=-";
            for (int i = 0; i < svid_remark_cnt.value(); i++) {
                str += svid_remarks[i].value() + Environment.NewLine;
            }
            str += "-=end=-\r\n" + svid_head.log() + Environment.NewLine + svid_farm.log() + "\r\nreserved2=";
            //if (reserved2!=null)
            //for (int i = 0; i < 100; i++) str += String.Format("{0:X} ", reserved2[i]);
            str += Environment.NewLine;
            if (subscriber.Count > 0) {
                for (int i = 0; i < 5; i++) {
                    str += "subscriber" + i.ToString() + "=" + subscriber[i].log() + Environment.NewLine;
                }
            }
            str += jobcnt.log() + Environment.NewLine;
            for (int i = 0; i < (int)jobcnt.value(); i++) {
                str += "job" + i.ToString() + "=" + jobs[i].log() + Environment.NewLine;
            }
    ...
    class MFGraphForm : MFCommon, IMFCommon
    {
        ...
        private MiaFile mf;
        public MFGraphForm(String name, int li, MiaFile mf)
            : base(name, li)
        {
            this.mf = mf;
        }
        ...
        public override string strval()
        {
            return "-=GraphForm=-\r\n" + reasons.log() + Environment.NewLine + workers.log() + Environment.NewLine + lost.log() + Environment.NewLine + "-=GraphForm_End=-";
        }
    }
    ...
    class MFArchiveForm : MFCommon, IMFCommon
    {
        ...
        public override string strval()
        {
            String str = "-=ArchiveForm=-\r\n" + size.log() + Environment.NewLine;
            for (int i = 0; i < size.value(); i++) {
                str += "plan" + i.ToString() + "=" + plans[i].log() + Environment.NewLine;
            }
            if (dver > 5.1) {
                str += dead.log() + Environment.NewLine;
            } else {
                str += dead2.log() + Environment.NewLine;
            }
